    ...
    def POST(self):
        ...
        if   msgType == 'event':
            ...
            if len(id) == 0:
                if len(content) > 50:
                    ...
                    if code != 200:
                        ...
                        id   = xml0[0][6][0].text
                        db.insert('info', id=id ,weixin_id=fromUser ,xiezhua_id=content)
                ...
                if   content == 'help':
                    ...
                    sendfrom = db.select('info',what="sendfrom"  ,where="weixin_id=$fromUser" ,vars=locals())
                    if sendfrom[0].sendfrom.encode('raw_unicode_escape') == content[8::]:
                    ...
                    code,xiezhua_xml = xiezhua(xiezhua_id[0].xiezhua_id,content,sendfrom[0].sendfrom.encode('raw_unicode_escape'),'talk')
                    if code == 200:
            ...
            if len(id) == 0:
                ...
                code,xiezhua_xml = xiezhua(xiezhua_id[0].xiezhua_id,'''[img]'''+picurl+'''[/img]''',sendfrom[0].sendfrom.encode('raw_unicode_escape'),'talk')
                if code == 200:
                    return render.weixin(fromUser,toUser,int(time.time()),xiezhua_xml)
                else:
